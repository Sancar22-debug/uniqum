        ...
        <div className="hidden md:grid md:grid-cols-2 xl:grid-cols-4 gap-5 mb-12">
          {directions.map((dir, index) => (
            <button
              ...
              className={`reveal-item relative overflow-hidden rounded-3xl p-6 text-left cursor-pointer transition-all duration-300 border border-[#0A2463]/15 bg-white ${activeIndex === index ? "ring-2 ring-[#0A2463] -translate-y-0.5 shadow-xl" : "hover:-translate-y-0.5 hover:shadow-md"
                }`}
            >
              <div
            ...
            <div className="flex gap-2">
              {directions.map((_, index) => (
                <button
                  ...
                  className={`h-2.5 rounded-full transition-all duration-300 ${activeIndex === index ? "bg-[#0A2463] w-8" : "bg-[#0A2463]/25 w-2.5"
                    }`}
